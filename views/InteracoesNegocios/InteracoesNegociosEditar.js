...
function EditarInteracaoNegocio() {
  const { id } = useParams();
  ...
  useEffect(() => {
    axios
      .get(`https://bacnkend.onrender.com/interacoesnegocios/get/${id}`)
      .then((response) => {
        ...
        setFormData({
          ...
          IDTIPOCONTACTO: interacaoData.IDTIPOCONTACTO,
        });
      })
      .catch((error) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(`https://bacnkend.onrender.com/interacoesnegocios/update/${id}`, formData)
      .then((response) => {
  ...
  return (
    <Container style={{ fontWeight: 'bold', textAlign: 'left' }}>
      ...
      <div className="interacao-editar">
        <h1 style={{ fontWeight: 'bold', color: '#15659F', textAlign: 'left' }}>Editar Interação de Negócio</h1>
...
export default EditarInteracaoNegocio;
